...
class Mage_Adminhtml_Cms_PageController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            /* START MILAN VORTEX TASK 2. */
            if(isset($data['page_heading']['delete']) && $data['page_heading']['delete'] == '1'){
            	if(!empty($data['page_heading']['value'])){
            		$path = Mage::getBaseDir('media') . DS;
            		if(@unlink($path.$data['page_heading']['value'])){
            			$data['page_heading'] = '';
            		}
            	}
            }
            
            if(isset($_FILES['page_heading']['name']) && !empty($_FILES['page_heading']['name'])) {
            	try {
            		$uploader = new Varien_File_Uploader('page_heading');
            		$uploader->setAllowedExtensions(array('jpg','jpeg','gif','png', 'svg'));
            		$uploader->setAllowRenameFiles(true);
            		$uploader->setFilesDispersion(true);
            		$path = Mage::getBaseDir('media') . DS;
            
            		$uploader->save($path, $_FILES['page_heading']['name']);
            		$_tmp_nfile = $uploader->getUploadedFileName();
            
            		$data['page_heading'] = $_tmp_nfile;
            
            	} catch(Exception $e) {
            
            	}
            } elseif(isset($data['page_heading']['value']) && !empty($data['page_heading']['value'])){
            	$data['page_heading'] = $data['page_heading']['value'];
            }
            /* END MILAN VORTEX TASK 2. */
